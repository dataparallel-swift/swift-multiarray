...
let benchmarks: @Sendable () -> Void = {
    ...
    func setup(_ n: Int) -> (Array<Zone>, MultiArray<Zone>) {
        ...
        var ys = MultiArray<Zone>(unsafeUninitializedCapacity: n)
    ...
    for (size, scaling) in configs {
        Benchmark("array/move/\(size)", configuration: config(scaling), closure: bench(readme1_array), setup: { setup(size).0 })
        Benchmark("array/unzip/\(size)", configuration: config(scaling), closure: bench(readme2_array), setup: { setup(size).0 })
        Benchmark("multiarray/move/\(size)", configuration: config(scaling), closure: bench(readme1_multiarray), setup: { setup(size).1 })
        Benchmark("multiarray/unzip/\(size)", configuration: config(scaling), closure: bench(readme2_multiarray), setup: { setup(size).1 })
    }
}
